  ...
  if (esAdmin) {
    ...
    return (
      <div className="p-6">
        ...
        {asesores.length > 0 && (
          <section className="mb-6">
            <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Por asesor (30 días)</h2>
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500 uppercase">Asesor</th>
                    <th className="text-right px-4 py-2.5 text-xs font-medium text-gray-500 uppercase">Pedidos</th>
                    <th className="text-right px-4 py-2.5 text-xs font-medium text-gray-500 uppercase">Ventas</th>
                    <th className="text-right px-4 py-2.5 text-xs font-medium text-gray-500 uppercase">Ticket prom.</th>
                    <th className="text-right px-4 py-2.5 text-xs font-medium text-gray-500 uppercase">Activos</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {asesores.map((a) => (
                    <tr key={a.asesor_id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium text-gray-900">{a.asesor_nombre}</td>
                      <td className="px-4 py-3 text-right text-gray-700">{a.pedidos_mes}</td>
                      <td className="px-4 py-3 text-right font-medium text-gray-900">{formatCOP(a.ventas_mes)}</td>
                      <td className="px-4 py-3 text-right text-gray-700">{formatCOP(a.ticket_promedio)}</td>
                      <td className="px-4 py-3 text-right text-gray-700">{a.pedidos_activos}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
