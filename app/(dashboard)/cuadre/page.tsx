        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100">
              <p className="text-sm font-semibold text-gray-900">Por cuenta</p>
            </div>
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {cuadre.porCuenta.map(c => (
                  <tr key={c.label}>
                    <td className="px-5 py-2.5 font-medium text-gray-800">{c.label}</td>
                    <td className="px-3 py-2.5 text-right text-gray-500">{c.venta  ? formatCOP(c.venta)   : '—'}</td>
                    <td className="px-3 py-2.5 text-right text-gray-500">{c.abono  ? formatCOP(c.abono)   : '—'}</td>
                    <td className="px-3 py-2.5 text-right text-gray-500">{c.cartera ? formatCOP(c.cartera) : '—'}</td>
                    <td className="px-5 py-2.5 text-right font-semibold text-gray-900">{formatCOP(c.total)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="px-5 py-3 border-b border-gray-100">
                <p className="text-sm font-semibold text-gray-900">Por asesor</p>
              </div>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50 text-xs text-gray-500 uppercase">
                ...
                <tbody className="divide-y divide-gray-50">
                  {cuadre.porAsesor.map(a => (
                    <tr key={a.asesor_id}>
                      ...
                      <td className="px-3 py-2.5 text-right text-gray-500">{a.venta   ? formatCOP(a.venta)   : '—'}</td>
                      <td className="px-3 py-2.5 text-right text-gray-500">{a.abono   ? formatCOP(a.abono)   : '—'}</td>
                      <td className="px-3 py-2.5 text-right text-gray-500">{a.cartera ? formatCOP(a.cartera) : '—'}</td>
            ...
            {cuadre.porSede.length > 1 && (
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className="px-5 py-3 border-b border-gray-100">
                  <p className="text-sm font-semibold text-gray-900">Consolidado por sede</p>
                </div>
                <table className="w-full text-sm">
                  <tbody className="divide-y divide-gray-50">
