...
import { precioEnPesos, TAX_USD, GANANCIA_USD } from '@/lib/utils/precios-dolar'
import {
  guardarTrmAction,
...
// tipo de artículo → valor USD → valor en pesos, calculado con la fórmula
// del negocio (USD + 7% tax + 40% ganancia, convertido con la TRM).
export function PreciosDolarClientPage({
  ...
  function agregar() {
    ...
    start(async () => {
      const r = await crearPrecioDolarAction({ tipo_articulo: nuevoTipo, valor_usd })
      if (!r.ok) { setError(r.error); return }
      setNuevoTipo(''); setNuevoUsd('')
      router.refresh()
    })
  }
  ...
  function guardarEdicion(id: string) {
    ...
    start(async () => {
      const r = await editarPrecioDolarAction(id, { tipo_articulo: editTipo, valor_usd })
      if (!r.ok) { setError(r.error); return }
      setEditandoId(null)
      router.refresh()
    })
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Valor en pesos = (USD + {Math.round(TAX_USD * 100)}% tax + {Math.round(GANANCIA_USD * 100)}% ganancia) × valor del dólar
        </p>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
          ...
          {nuevoUsdNum > 0 && trm > 0 && (
            <p className="text-xs text-gray-500">
              {formatUSD(nuevoUsdNum)} quedaría en <span className="font-bold text-gray-900">{formatCOP(precioEnPesos(nuevoUsdNum, trm))}</span>
            </p>
          )}
          <button
            onClick={agregar}
            disabled={guardando || !nuevoTipo.trim() || !(nuevoUsdNum > 0)}
                  ...
                  <tr key={p.id} className="hover:bg-gray-50/60">
                    <td className="px-5 py-2.5">
                      {editando ? (
                        ...
                        <span className="font-medium text-gray-900">{p.tipo_articulo}</span>
                      )}
                    </td>
                    ...
                    <td className="px-5 py-2.5 text-right font-bold text-green-700">
                      {trm > 0
                        ? formatCOP(precioEnPesos(editando ? (editUsdNum || p.valor_usd) : p.valor_usd, trm))
                        : '—'}
                    </td>
                    {esAdmin && (
                      <td className="px-3 py-2.5 text-right whitespace-nowrap">
                        {editando ? (
                          ...
                          <span className="inline-flex items-center gap-2.5">
                            <button
                              onClick={() => { setEditandoId(p.id); setEditTipo(p.tipo_articulo); setEditUsd(String(p.valor_usd)) }}
