-- Migration 048: Agregar codigo a pedido_items y guardarlo en articulos desde los RPCs

-- 1. Columna codigo en pedido_items
alter table pedido_items add column if not exists codigo text;

-- 2. Backfill: copiar codigo desde articulos donde ya estaba guardado
update pedido_items pi
set    codigo = a.codigo
from   articulos a
where  pi.articulo_id = a.id
  and  pi.codigo is null
  and  a.codigo is not null;

-- 3. registrar_venta_inmediata: guarda codigo en articulos y en pedido_items
  ...
  loop
    ...
    if v_articulo is null and v_marca is not null and v_nombre is not null then
      ...
      do update set
        ...
        -- solo actualizar codigo si el articulo aun no tiene uno
...
-- 4. crear_factura_venta_local: idem
