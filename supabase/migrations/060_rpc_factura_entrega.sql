-- Migration 060: RPCs de factura con tipo de entrega integrado
--
-- Reescribe crear_factura y crear_factura_venta_local para que, además de
-- crear la factura y sus abonos, resuelvan la entrega en un solo paso:
--   - DOMICILIO: crea el domicilio, el recaudo de la mensajería y, si TB
--     asume el domicilio, el gasto + la deuda con la mensajería.
--   - ENVÍO: crea el gasto (si lo paga cliente o TB; contra entrega no genera nada).
--
-- Helper _entrega_factura concentra la lógica para no duplicarla.
...
begin
  ...
  if p_tipo_entrega = 'domicilio' and p_mensajeria is not null then
    ...
    if p_quien_paga = 'cliente' then
      ...
      v_cobrar := p_total + coalesce(p_valor, 0);
    else
    ...
    values (
      p_factura_id, p_mensajeria, p_total, coalesce(p_valor, 0), v_cobrar,
    ...
    -- Recaudo: el mensajero debe traer el total de la factura a TB
    if p_total > 0 then
      insert into pagos_mensajeria (mensajeria, tipo, monto, fecha, factura_id, responsable_id, estado, concepto, notas)
      values (p_mensajeria, 'deuda', p_total, current_date, p_factura_id, p_asesor_id, 'pendiente', 'recaudo',
              'Recaudo factura ' || p_numero);
    end if;
...
DROP FUNCTION IF EXISTS public.crear_factura(uuid, uuid, uuid, date, uuid[], text, jsonb, integer, integer);
...
DROP FUNCTION IF EXISTS public.crear_factura_venta_local(uuid, uuid, uuid, date, jsonb, jsonb, integer, integer, text);
