-- Restaurar versiones completas de los RPCs de factura con envío + descuento + cuenta + metodo.
-- La migración 046 había dejado versiones sin envío/descuento; aquí se restauran completas.
DROP FUNCTION IF EXISTS public.crear_factura(uuid, uuid, uuid, date, uuid[], text, integer, text, uuid);
DROP FUNCTION IF EXISTS public.crear_factura_venta_local(uuid, uuid, uuid, date, jsonb, integer, text, uuid, text);

-- crear_factura: pedidos existentes + envío/descuento + abono con metodo y cuenta
...
-- crear_factura_venta_local: venta del local + envío/descuento + abono con metodo y cuenta
