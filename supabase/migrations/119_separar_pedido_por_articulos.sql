-- Migración 119: separar un pedido en un pedido por artículo (TR6835 → TR6835-1,
-- TR6835-2, …) para poder facturar/enviar solo los artículos que ya llegaron.
-- El pedido original se convierte en la parte -1 (conserva su id, historial y
-- código de barras interno); las demás partes son pedidos nuevos con el mismo
-- cliente, estado y fecha. Los abonos se reparten llenando las partes en orden
-- (los más antiguos primero) sin que ninguna parte quede abonada por encima de
-- su total; si un abono queda a caballo entre dos partes, se divide en dos
-- registros que suman lo mismo (la caja no se mueve). Las compras ya asignadas
-- se llevan a la parte de su artículo (por artículo del catálogo o código).

create or replace function separar_pedido_por_articulos(p_pedido_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  for r in (
    ...
    if v_i = 1 then
      ...
      update compra_items ci set pedido_id = v_new_id
      where ci.id in (
...
revoke all on function separar_pedido_por_articulos(uuid) from public, anon;
grant execute on function separar_pedido_por_articulos(uuid) to authenticated;
