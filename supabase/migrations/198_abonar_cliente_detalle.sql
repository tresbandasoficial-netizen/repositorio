-- Migración 198: abonar_cliente devuelve el DETALLE del reparto
--
-- Pedido de Johan: al registrar un abono a la deuda, ver a cuál pedido o
-- factura se fue cada parte (el reparto es del más antiguo al más nuevo y
-- confundía no ver dónde cayó la plata). Misma lógica de la 149 — solo se
-- agrega el detalle [{ref, monto, tipo}] al jsonb de respuesta.
...
begin
  ...
  for v_ped in
    select p.id, p.total, p.factura_id, p.numero_orden
    from pedidos p
  ...
  loop
    ...
    if v_ped.factura_id is not null then
      ...
      v_detalle := v_detalle || jsonb_build_object(
        'ref', coalesce(v_num_fac, 'Factura'), 'monto', v_aplicar, 'tipo', 'factura');
      ...
      v_detalle := v_detalle || jsonb_build_object(
        'ref', v_ped.numero_orden, 'monto', v_aplicar, 'tipo', 'pedido');
    end if;
  end loop;

  return jsonb_build_object(
    'aplicado', p_monto - v_restante,
    'sobrante', v_restante,
    'detalle',  v_detalle
  );
end;
