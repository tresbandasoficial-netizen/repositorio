-- Migration 071: Fechas en hora Bogotá (UTC-5) en la base de datos
--
-- Problema: las funciones y los DEFAULT de columnas usaban current_date, que
-- en PostgreSQL devuelve la fecha del servidor en UTC. Entre las 19:00 y las
-- 23:59 hora Bogotá, current_date ya es el día siguiente. Resultado: pagos,
-- recaudos, domicilios y gastos creados de noche quedaban con fecha del día
-- siguiente y descuadraban el cierre diario.
--
-- Solución:
--   1. Función hoy_bogota() que devuelve la fecha local de Colombia.
--   2. DEFAULT de las columnas de fecha apuntando a hoy_bogota().
--   3. Recrear las funciones que usaban current_date para usar hoy_bogota().

-- ── 1. Helper de fecha local ─────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.hoy_bogota()
RETURNS date LANGUAGE sql STABLE AS $$
  SELECT (now() AT TIME ZONE 'America/Bogota')::date;
$$;

-- ── 2. DEFAULT de columnas de fecha (cubre los INSERT que no pasan fecha) ─────
ALTER TABLE pagos            ALTER COLUMN fecha SET DEFAULT hoy_bogota();
ALTER TABLE pagos_factura    ALTER COLUMN fecha SET DEFAULT hoy_bogota();
ALTER TABLE pagos_mensajeria ALTER COLUMN fecha SET DEFAULT hoy_bogota();
ALTER TABLE gastos           ALTER COLUMN fecha SET DEFAULT hoy_bogota();
ALTER TABLE domicilios       ALTER COLUMN fecha SET DEFAULT hoy_bogota();

-- ── 3. Recrear funciones reemplazando current_date por hoy_bogota() ──────────
