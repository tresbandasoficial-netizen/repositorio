-- Fix: anular_factura no pasaba usuario_id al insertar movimientos_inventario
-- de reversa de ventas locales → violación NOT NULL en la columna.
-- Se agrega p_usuario_id (con default null para no romper llamadores anteriores)
-- y se usa coalesce(p_usuario_id, m.usuario_id) para atribuir el movimiento.
...
begin
  ...
  insert into movimientos_inventario
    (articulo_id, talla, sede_id, delta, tipo, pedido_id, costo_unitario_cop, notas, usuario_id)
  select m.articulo_id, m.talla, m.sede_id, -m.delta, 'entrada', m.pedido_id,
         ...
         coalesce(p_usuario_id, m.usuario_id)
