-- Migration 062: Método de pago "Recaudo Mensajería"
--
-- Reemplaza el modelo de "contra entrega" (que asumía que el mensajero cobraba
-- todo el saldo automáticamente) por un método de pago explícito:
--
--   "Recaudo Mensajería" es una línea de pago más dentro de un pago mixto.
--   Ej:  Pedido $390.000  =  Addi $370.000  +  Recaudo Mensajería $20.000
--        → la factura queda PAGADA, el cliente sin saldo
--        → la mensajería le debe $20.000 a TB (concepto='recaudo')
--
-- Cambios:
--   - El recaudo de la mensajería se crea por CADA línea de abono con
--     metodo='recaudo_mensajeria' (monto y mensajería explícitos), no por saldo.
--   - _entrega_factura ya NO crea el recaudo automático por saldo; recibe el
--     total recaudado por mensajería para calcular el valor a cobrar del
--     domicilio (recaudo + domicilio si lo paga el cliente).

-- ── Helper: procesa la entrega de una factura ya creada ──────────────────────
...
CREATE FUNCTION public._entrega_factura(
  ...
  p_sede_id      uuid,
  p_asesor_id    uuid,
  p_recaudo      integer,   -- total que la mensajería recauda (líneas recaudo_mensajeria)
  p_tipo_entrega text,
  p_mensajeria   text,
  p_valor        integer,
  p_quien_paga   text,
...
declare
  ...
  v_cli    record;
begin
  -- ── DOMICILIO (mensajero local) ──
  if p_tipo_entrega = 'domicilio' and p_mensajeria is not null then

    if p_quien_paga = 'cliente' then
      -- El cliente paga el domicilio: el mensajero cobra recaudo + domicilio
      v_cobrar := coalesce(p_recaudo, 0) + coalesce(p_valor, 0);
    else
      -- TB asume el domicilio: el mensajero solo cobra el recaudo
      v_cobrar := coalesce(p_recaudo, 0);
      if coalesce(p_valor, 0) > 0 then
        -- Gasto para la P&L / página de gastos
        insert into gastos (categoria, valor, sede_id, responsable_id, fecha, origen, origen_id, observacion)
        values ('domicilios', p_valor, p_sede_id, p_asesor_id, current_date, 'domicilio', p_factura_id,
                'Domicilio asumido por TB · ' || p_mensajeria);
        -- Deuda con la mensajería (TB le debe el domicilio)
        insert into pagos_mensajeria (mensajeria, tipo, monto, fecha, factura_id, responsable_id, estado, concepto, notas)
        values (p_mensajeria, 'deuda', p_valor, current_date, p_factura_id, p_asesor_id, 'pendiente', 'domicilio_tb',
    ...
    -- Crear el domicilio (aparece automáticamente en el módulo de Domicilios).
    -- El recaudo (pagos_mensajeria concepto='recaudo') se crea en el loop de
    -- abonos, no aquí, para no duplicarlo.
    insert into domicilios (
      factura_id, mensajeria, valor_pedido, valor_domicilio, valor_a_cobrar,
      origen, estado, asesor_id, fecha, metodo_pago,
      cliente_nombre, cliente_telefono, direccion, cobrar_al_cliente, tipo_cobro, numero_pedido
    )
    values (
      p_factura_id, p_mensajeria, coalesce(p_recaudo, 0), coalesce(p_valor, 0), v_cobrar,
      'factura', 'pendiente', p_asesor_id, current_date, 'efectivo',
  ...
  -- ── ENVÍO (empresa de paquetería) ──
  elsif p_tipo_entrega = 'envio'
        and coalesce(p_valor, 0) > 0
        and coalesce(p_quien_paga, '') <> 'contra_entrega' then
    insert into gastos (categoria, valor, sede_id, responsable_id, fecha, origen, origen_id, observacion)
    values ('envio', p_valor, p_sede_id, p_asesor_id, current_date, 'envio', p_factura_id,
...
-- ── crear_factura: pedidos existentes + entrega ──────────────────────────────
...
begin
  ...
  if p_abonos is not null then
    ...
    loop
      ...
      -- Recaudo Mensajería: la mensajería cobró este valor al cliente y se lo
      -- debe a TB. La factura ya lo cuenta como pagado (pagos_factura arriba).
      if v_abono->>'metodo' = 'recaudo_mensajeria' and nullif(v_abono->>'mensajeria', '') is not null then
        insert into pagos_mensajeria (mensajeria, tipo, monto, fecha, factura_id, responsable_id, estado, concepto, notas)
        values (v_abono->>'mensajeria', 'deuda', (v_abono->>'monto')::integer, current_date, v_factura_id, p_asesor_id, 'pendiente', 'recaudo',
  ...
  -- explícito (+ domicilio si lo paga el cliente).
...
-- ── crear_factura_venta_local: venta del local + entrega ─────────────────────
